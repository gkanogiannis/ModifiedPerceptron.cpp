...
inline void ZshCompletionOutput::printArg(Arg* a) {
	...
	std::cout << a->getName() << ':';
	std::map<std::string, std::string>::iterator compArg = common.find(
			a->getName());
...
inline std::string ZshCompletionOutput::getMutexList(CmdLineInterface& _cmd,
		Arg* a) {
